...
// During discussions the consensus has been that it will not matter right now which value is used for ConfigureAwait
...
namespace TransformsAI.Unity.WebGL
{
    public sealed class UnityWebGlHttpHandler : HttpMessageHandler
    {
        ...
        private static readonly JsValue Fetch = JsRuntime.GetGlobalValue("fetch");
        private static readonly JsValue Window = JsRuntime.GetGlobalValue("window");
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // UnityEngine.Debug.Log($"--->>>request.Content {request.Content} {request.Content.GetHashCode()}");
                ...
                var httpResponse = new HttpResponseMessage((HttpStatusCode)status.Status);
                ...
                {
                    if (respHeaders == null) return httpResponse;
